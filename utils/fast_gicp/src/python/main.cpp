...
Eigen::Matrix<double, -1, 3> downsample(const Eigen::Matrix<double, -1, 3>& points, double downsample_resolution) {
  ...
  // ! pcl::ApproximateVoxelGrid::filter() does not remove NaN points.
  pcl::PointCloud<pcl::PointXYZ>::iterator it = filtered->points.begin();
  while (it != filtered->points.end()) 
  {
    float x, y, z;
    x = it->x;
    y = it->y;
    ...
    if (!pcl_isfinite(x) || !pcl_isfinite(y) || !pcl_isfinite(z))
      {
        // std::cerr << "down error: nan/inf points " << x << " " << y << " " << z << std::endl;
        // std::cout << "point value: " << x << " " << y << " " << z << std::endl;
        it = filtered->points.erase(it);
      }
    else
      ++it;
  }  
...
Eigen::Matrix4d align_points(
  ...
  if(downsample_resolution > 0.0) {
    ...
    while (it != filtered->points.end()) 
    {
      ...
      if (!pcl_isfinite(x) || !pcl_isfinite(y) || !pcl_isfinite(z))
        {
          // std::cerr << "align error: nan/inf points " << x << " " << y << " " << z << std::endl;
          // std::cout << "point value: " << x << " " << y << " " << z << std::endl;
          it = filtered->points.erase(it);
        }
      else
...
PYBIND11_MODULE(pygicp, m) {
  ...
  m.def("align_points", &align_points, "align two point sets",
    ...
    // py::arg("neighbor_search_method") = "DIRECT_RADIUS",
  ...
  py::class_<LsqRegistration, std::shared_ptr<LsqRegistration>>(m, "LsqRegistration")
    ...
    .def("get_fitness_score", [] (LsqRegistration& reg, const double max_range) { return reg.getFitnessScore(max_range); }, py::arg("max_range") = 1.0)
    .def("align",
